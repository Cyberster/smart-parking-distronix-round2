...
// Obtain status information of a bay: GET /bay/:lot_name/:bay_name/status
bayRouter.get('/:lot_name/:bay_name/status', async (req, res, next) => {
    console.log(req.body)

    let lot_name = req.params.lot_name
    let bay_name = req.params.bay_name
    //let status = req.params.bay_name

    try {
        let result = await db.getBayStatusByGlobalName(lot_name, bay_name)
        // res.json(result)
