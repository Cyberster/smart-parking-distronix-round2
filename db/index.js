...
db.addSensor = (uuid, is_occupied, is_faulty) => {
	return new Promise((resolve, reject) => {
		connPool.query('INSERT INTO sensor (`uuid`, `is_occupied`, `is_faulty`) VALUES (?, ?, ?) ', 
													[uuid, is_occupied, is_faulty], (err, results) => {
			result = {
				status: 'error'
			}
...
db.updateSensor = (id, is_occupied, is_faulty) => {
	return new Promise((resolve, reject) => {
		connPool.query('UPDATE sensor SET `is_occupied` = ?, `is_faulty` = ? WHERE `id` = ?', 
												[is_occupied, is_faulty, id], (err, results) => {
			result = {
				status: 'error'
			}
...
// Update existing sensor details in the database
db.updateSensorByUUID = (uuid, is_occupied, is_faulty) => {
	return new Promise((resolve, reject) => {
		connPool.query('UPDATE sensor SET `is_occupied` = ?, `is_faulty` = ? WHERE `uuid` = ?', 
												[is_occupied, is_faulty, uuid], (err, results) => {
			result = {
				status: 'error'
			}
...
// Add status to the database
db.addStatusByUUID = (uuid, timestamp, is_occupied, is_faulty) => {
	return new Promise((resolve, reject) => {
		connPool.query('INSERT INTO status (`sensor_id`, `timestamp`, `is_occupied`, `is_faulty`) \
									VALUES ((SELECT id FROM sensor WHERE uuid = ?), ?, ?, ?) ', 
									[uuid, timestamp, is_occupied, is_faulty], (err, results) => {
...
// Get bay details by global name
db.getBayStatusByGlobalName = (lot_name, bay_name) => {
	return new Promise((resolve, reject) => {
		connPool.query('SELECT s.is_occupied, s.is_faulty\
						FROM lot l, bay b, sensor s \
						WHERE b.lot_id = l.id AND b.sensor_id = s.id AND l.name = ? AND b.name = ?', 
						[lot_name, bay_name], (err, results) => {
			result = {
				data: [],
				status: 'error'
			...
			result.data = results[0]
			// console.log('typeof', typeof result.data.is_occupied)
			// console.log(result.data.is_occupied)
			// console.log(result.data.is_faulty)
			result.status = results.length > 0 ? 'success' : 'error';
